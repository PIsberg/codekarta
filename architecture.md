...
> *This document was generated by code-karta documenting itself.*  
...
## 3. Input Parser Dispatch — Exception-Flow Sequence Diagram

Generated from `JavaSourceInputParser.java` (default mode — includes exception propagation edges):

```
java -jar karta.jar --input code-karta-input/src/main/java/com/karta/input/JavaSourceInputParser.java --output docs/diagrams
```

`JavaSourceInputParser` is the facade for Tier 1. This diagram shows its method-call sequence and any try/catch boundaries surfaced as `Group` nodes, with `EXCEPTION_PROPAGATION` edges overlaid.

![JavaSourceInputParser exception-flow sequence diagram](docs/diagrams/javasourceinputparser-sequence-diagram.svg)
