...
| Using the Maven plugin | [`docs/MAVEN-PLUGIN.md`](docs/MAVEN-PLUGIN.md) |
...
- **Everything shipped compiles for Java 17** (`<library.release>` in the parent pom), tests
  compile for 21, and the build itself needs a JDK between 21 and 25, which the enforcer checks.
  Consumers on 17 are the reason; the one runtime exception is the `elk` layout, which needs 21
  and falls back to `simple` below it.
- **`java.util.logging.Logger` only** — no SLF4J/Logback, to avoid binding conflicts across
  modules.
...
<project_guardrails>
  ...
  <audit_requirements>
    <file path="se.deversity.codekarta.maven.GenerateMojo">
      <vulnerability_check>Path traversal</vulnerability_check>
      <vulnerability_check>Unauthorized file write</vulnerability_check>
    </file>
  </audit_requirements>
